...
export class ReadFileTool implements AgentTool {
    ...
    async execute(input: { filePath: string }, onProgress?: (data: string) => void): Promise<ToolResult> {
...
export class WriteFileTool implements AgentTool {
    ...
    schema = {
        ...
        properties: {
            ...
            contentLines: { 
                type: 'array', 
                items: { type: 'string' },
                description: 'An array of strings, where each string is a line of the file. Do NOT include manual \\n escapes.' 
            }
        },
    ...
    async execute(input: { filePath: string, contentLines: string[] }, onProgress?: (data: string) => void): Promise<ToolResult> {
        if (!this.workspaceRoot) {
            return { success: false, output: '', error: 'No workspace folder is open. Please open a folder in VS Code first.' };
        }
        try {
