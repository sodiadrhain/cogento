...
export class Planner {
    constructor(private provider: LLMProvider, private tools: AgentTool[]) {}

    async generatePlan(history: Message[], systemPrompt: string): Promise<{ plan: PlanStep, rawText: string }> {
...
3. Do NOT wrap your response in markdown blocks like \`\`\`json.`;
        ...
        const dummyToken = { isCancellationRequested: false, onCancellationRequested: () => { return { dispose: () => {} }; } };
        ...
        try {
            ...
            const fallback: PlanStep = { reasoning: 'Failed to parse', isFinished: true, finalAnswer: 'I encountered an error planning the next step. The JSON response was invalid. Please try again.' };
