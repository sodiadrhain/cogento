...
export type AgentEventType = 'start' | 'reasoning' | 'tool_start' | 'tool_progress' | 'tool_end' | 'approval' | 'answer' | 'error';
...
5. **Think Step-by-Step**: Use your tools logically to satisfy the user's request. Always explain your intent before calling a tool.`;
        ...
        while (!isFinished && iterations < maxIterations) {
            ...
            if (plan.tool_name) {
                ...
                if (tool.requiresApproval) {
                    if (this.askApproval) {
                        ...
                        if (!approved) {
                            ...
                            this.history.push({ role: 'assistant', content: rawText });
                            this.history.push({ role: 'system', content: `User DENIED execution of ${tool.name}. Explain alternative or abort.` });
                            continue;
                        }
                    } else {
                        this.onEvent({ type: 'error', text: `Error: Tool ${tool.name} requires approval but no approval handler is registered.` });
                        continue;
                    }
                }

                this.onEvent({ type: 'tool_start', text: `Running tool: ${plan.tool_name}`, data: { toolName: plan.tool_name, toolInput: plan.tool_input } });
                ...
                if (result.success) {
                    this.onEvent({ type: 'tool_end', text: `Tool succeeded. Output length: ${result.output.length}`, data: result });
                } else {
                ...
                this.history.push({ role: 'assistant', content: rawText });
                this.history.push({ role: 'system', content: `You did not call a tool and did not set isFinished to true. Please proceed.` });
            }
        }
